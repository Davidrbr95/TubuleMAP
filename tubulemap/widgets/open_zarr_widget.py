...
class ZarrLoaderWidget(QWidget):
    ...
    def _auto_adjust_contrast(self, layer, source_meta=None):
        ...
        # Prefer level-0 source data (raw intensities)
        ...
        for candidate in unique_candidates:
            limits = self._sample_contrast_limits(
                candidate
            )
        ...
        # Fallback to dtype limits when sampling cannot
        # estimate a valid range.
        for candidate in unique_candidates:
            limits = self._dtype_contrast_limits(
                candidate
            )
    ...
    def _set_source_metadata(
        ...
        *,
        ...
        """
        Attach resolved source metadata to loaded layers
        and optionally normalize contrast.
        """
        for layer in layers or []:
            if hasattr(layer, "metadata"):
                layer.metadata[
                    "tubulemap_source_resolution"
                ] = source_meta
            ...
            if "image" in layer_type:
                self._auto_adjust_contrast(
                    layer,
                    source_meta=source_meta,
                )

    def _remove_new_layers(
        self,
        existing_layers,
    ):
        """
        Remove layers partially added by a failed open operation.
        """
        existing_ids = {
            id(layer)
    ...
    def _open_with_napari_default(
        self,
        directory,
    ):
        """
        Use napari default reader resolution to match
        ...
        try:
            ...
            # viewer.open() can fail after a layer has already
            # been inserted into the viewer. Remove those partial
            # layers before trying another reader.
            self._remove_new_layers(
                existing_layers
            )
            return None
    ...
    def load_zarr_volume(self):
        ...
        try:
            source_meta = inspect_zarr_source(
                directory
            )
        except Exception as exc:
            QMessageBox.critical(
                self,
                "Load Failed",
                f"Could not inspect zarr source:\n{exc}",
            )
            return

        # First try napari's default reader resolution path;
        # this most closely matches drag-and-drop.
        opened_layers = (
            self._open_with_napari_default(
                directory
            )
        )

        if opened_layers:
            # Keep plugin-selected contrast behavior to avoid
            # dark/black multiscale OME views.
        ...
        if source_meta.get(
            ...
            try:
                opened_layers = self.viewer.open(
                    directory,
                    plugin="napari-ome-zarr",
                )

            except Exception as exc:
                # Remove any layers that were partially inserted
                # before the reader raised the exception.
                self._remove_new_layers(
                    existing_layers
                )
                ...
                QMessageBox.critical(
                    self,
                    "OME-Zarr Load Failed",
                    "Failed to open with plugin "
                    "'napari-ome-zarr'.\n\n"
                    f"Error: {exc}",
                )
                return

            # For OME, keep plugin-selected rendering defaults
            # (contrast/gamma/scale).
            self._set_source_metadata(
                opened_layers,
                source_meta,
                adjust_contrast=False,
        ...
        if not levels:
            QMessageBox.critical(
                self,
                "Load Failed",
                "No levels were found in the selected "
                "zarr source.",
            )
            return
